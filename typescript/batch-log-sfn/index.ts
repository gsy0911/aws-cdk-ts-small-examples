...
export class BatchSfnStack extends cdk.Stack {
  constructor(app: cdk.App, id: string, params: IParameters, props?: cdk.StackProps) {
    super(app, id, props);
		...
		/** */
		const instanceProile = new iam.CfnInstanceProfile(this, `${params.environment}-instance-profile`, {
		...
		const batchComputeEnvironment = new batch.ComputeEnvironment(this, `${params.environment}-batch-environment`, {
			...
			computeResources: {
				...
				instanceRole: instanceProile.attrArn,
...
new BatchSfnStack(app, "BatchSfn", parameters);
app.synth();
